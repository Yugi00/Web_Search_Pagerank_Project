...
vector = gensim.downloader.load("glove-twitter-200")
...
class WebGraph():
    ...
    def search(self, pi, query='', max_results=10):
        ...
        for term in terms:
            if term[0] != '-':
                syns += syns_and_score(term)

        n = self.P.shape[0]
        values,indices = torch.topk(pi,n)

        urls = [self._index_to_url(index.item()) for index in indices]
        p_ranks = [val.item() for val in values]
        if query != '':
            scores = [0 for x in urls]
            for i, url in enumerate(urls):
                for word,simscore in syns:
                    scores[i] += url.count(word) * simscore**50
        else:
            scores = [1 for x in urls]

        rank = [(urls[i],p_ranks[i]*scores[i]) for i in range(len(scores))]
        rank.sort(key=lambda a: a[1],reverse=True)
        match = 0

        for i in range(n):
            if match >= max_results:
                break
            if url_satisfies_query(rank[i][0],query):
                logging.info(f'rank={match} ranking={rank[i][1]:0.4e} url={rank[i][0]}')
                match += 1
            
def synonyms(term):
    syns_list = vector.most_similar(term)
    if len(syns_list) > 5:
            syns_list = syns_list[:5]
    sims = [pair[0] for pair in syns_list]
    return sims

def syns_and_score(term):
